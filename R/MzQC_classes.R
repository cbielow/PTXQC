...
#' i.e. 
#'      if (!isValid(a) || !isValid(b)) doStuff()
#'      is harder to read than
#'      if (!isValid(a,b)) doStuff()
#'
#' @examples 
#'   isValid(MzQCcvParameter$new("QC:4000059"))       # FALSE
#'   isValid(list(MzQCcvParameter$new("QC:4000059"))) # FALSE
#'   isValid(list(MzQCcvParameter$new("QC:4000059", "Number of MS1 spectra"))) # TRUE
#'   isValid(list(MzQCcvParameter$new("QC:4000059", "Number of MS1 spectra")), MzQCcvParameter$new()) # FALSE
#'
isValid = function(x, ...)
{
  # anchor
...
#'  data_recon = fromData(MzQCcvParameter, list(jsonlite::fromJSON(jsonlite::toJSON(data))))
#'  data_recon
#'
fromData = function(mzqc_class, data)
{
...
MzQCDateTime = setRefClass(
  ...
  methods = list(
    ...
    {
      if (!isValid(.self)) stop(paste0("Object of class '", class(.self), "' is not in a valid state for writing to JSON"))
...
#' 
#' @examples 
#'   MzQCcontrolledVocabulary$new("Proteomics Standards Initiative Quality Control Ontology",
#'                                "https://github.com/HUPO-PSI/qcML-development/blob/master/cv/v0_1_0/qc-cv.obo",
#'                                "0.1.0")
#'   isValid(MzQCcontrolledVocabulary$new("Proteomics Standards Initiative Quality Control Ontology"))
#'    
...
#'                       c(0.2959, 5969.8172))
#'   isValid(MzQCcvParameter$new("MS:0000000"))
#'    
...
MzQCinputFile = setRefClass(
  ...
  methods = list(
    ...
    isValid = function(.self)
    {
      ...
      return(isValid(.self$fileProperties)) ## TRUE for empty list, which is ok
    },
    toJSON = function(.self, ...)
    {
      if (!isValid(.self)) stop(paste0("Object of class '", class(.self), "' is not in a valid state for writing to JSON"))
    ...
    fromData = function(.self, data)
    {
      ...
      .self$fileProperties = fromData(MzQCcvParameter, data$fileProperties) ## for lists, call the free function
    }
  )
)
...
xdata
class(fromData(MzQCcvParameter, xdata$fileProperties)) == "list"
...
MzQCmetadata = setRefClass(
  ...
  methods = list(
    ...
    {
      # force evaluation of all fields by '+'
      if (isUndefined(.self$label) + !isValid(.self$inputFiles, .self$analysisSoftware, .self$cvParameters)) return(FALSE)
      return(TRUE)
    },
    toJSON = function(.self, ...)
    {
    ...
    fromData = function(.self, data)
    {
      ...
      .self$inputFiles = fromData(MzQCinputFile, data$inputFiles)
      .self$analysisSoftware = fromData(MzQCanalysisSoftware, data$analysisSoftware)
      .self$cvParameters = fromData(MzQCcvParameter, data$cvParameters)
    }
  )
)
...
MzQCqualityMetric = setRefClass(
  ...
  methods = list(
    ...
    fromData = function(.self, data)
    {
      ...
      .self$unit = fromData(MzQCcvParameter, data$unit) ## if data$unit is empty, or NA, the empty list will be returned
    }
  )
)
...
MzQCbaseQuality = setRefClass(
  ...
  methods = list(
    ...
    isValid = function(.self)
    {
      if (!isValid(.self$metadata, .self$qualityMetrics)) return(FALSE)
      return(TRUE)
    },
    toJSON = function(.self, ...)
    {
    ...
    fromData = function(.self, data)
    {
      ...
      .self$qualityMetrics = fromData(MzQCbaseQuality, data$qualityMetrics) ## if data$qualityMetrics is empty, or NA, the empty list will be returned
    }
  )
)
  ...
  methods = list(
    ...
    isValid = function(.self)
    {
      ...
      if (isUndefined(.self$version) +
          !isValid(.self$creationDate, .self$runQualities, .self$setQualities, .self$controlledVocabularies)) return(FALSE)
    ...
    {
      .self$version = data$version
      .self$creationDate = fromData(MzQCDateTime, data$creationDate)
      ...
      .self$runQualities = fromData(MzQCbaseQuality, data$runQualities) ## if data$runQualities is empty, or NA, the empty list will be returned
      .self$setQualities = fromData(MzQCbaseQuality, data$setQualities) ## if data$setQualities is empty, or NA, the empty list will be returned
      .self$controlledVocabularies = fromData(MzQCcontrolledVocabulary, data$controlledVocabularies) ## if data$controlledVocabularies is empty, or NA, the empty list will be returned
    }
  )
)
setMethod('asJSON', 'MzQCbaseQuality', function(x, ...) x$toJSON(...))