...
createReport = function(txt_folder = NULL, mztab_file = NULL, yaml_obj = list(), report_filenames = NULL)
{
  ...
  if (enabled_evidence)
  {
    ...
    if (("retention.time.calibration" %in% colnames(df_evd)))
    {
      ...
      {
        lst_qcMetrics[["qcMetric_EVD_MBRAlign"]]$setData(df_evd, param_EV_MatchingTolerance, mq$fn_map$raw_file_mapping)
...
if (enabled_msms)
{
  ...
  ##
  lst_qcMetrics[["qcMetric_MSMS_MSMSDecal"]]$setData(df_msms = df_msms, fc_raw_files = mq$fn_map$raw_file_mapping$to)
...
hm = getQCHeatMap(lst_qcMetrics, raw_file_mapping = mq$fn_map$raw_file_mapping)
#print(hm[["plot"]])
...
pl_nameMapping = mq$fn_map$plotNameMapping()
...
mq$fn_map$writeMappingFile(rprt_fns$filename_sorting)
