...
qcMetric_PG_RawInt = qcMetric$new(
  helpText = 
...
qcMetric_PG_LFQInt = qcMetric$new(
  helpText = 
...
qcMetric_PG_ITRAQInt = qcMetric$new(
  helpText = 
...
reporter intensities are detected.",
  workerFcn=function(.self, df_pg, lst_cols, MAP_pg_groups)
  {
    ...
    for (cond in names(lst_cols))
    {
      ...
      #print(lpl)
      if (!inherits(pl, "try-error")) plots[[cnt]] = pl;
      cnt = cnt + 1;
    }
...
qcMetric_PG_Ratio = qcMetric$new(
  helpText = 
