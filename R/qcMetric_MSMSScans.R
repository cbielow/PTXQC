...
",
    ...
    {
      ## completeness check
      if(!checkInput(c("fc.raw.file", "retention.time", "scan.event.number", "rRT"), colnames(df_msmsScans))) return()
...
Heatmap score [MS<sup>2</sup> Scans: Intensity]: Linear score (0-100%) between 3 < (TIC / BP) < 10. 
    ...
    {
      ## completeness check
      if(!checkInput(c("fc.raw.file", "total.ion.current", "base.peak.intensity"), colnames(d_msmsScan))) return ()
...
",
    workerFcn = function(.self, df_msmsScans, threshold_iit)
    {
      ...
      if(!checkInput(c("fc.raw.file", "ion.injection.time", "rRT"), colnames(df_msmsScans))) return ()
...
",
    ...
    {
      ## completeness check
      if(!checkInput(c("fc.raw.file", "scan.event.number"), colnames(df_msmsScans))) return ()
...
",
    ...
    {
      ## completeness check
      if(!checkInput(c("fc.raw.file", "scan.event.number", "identified"), colnames(df_msmsScans))) return()
...
",
    ...
    {
      ## completeness check
      if(!checkInput(c("fc.raw.file", "dp.modification", "dp.aa", "identified"), colnames(d_msmsScan))) return()
